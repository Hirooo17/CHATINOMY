...
function App() {
  ...
  useEffect(() => {
    ...
    newSocket.on('partnerFound', ({ roomId: newRoomId, partnerName: receivedPartnerName }) => {
      setRoomId(newRoomId)
      setChatState('chatting')
      setPartnerName(receivedPartnerName || 'Stranger') // Use the received partner name
      setMessages([{ 
        type: 'system', 
        message: `Connected to ${receivedPartnerName || 'Stranger'}! Say hello!`,
        timestamp: new Date()
      }])
    })
  ...
  }, [partnerName]) // Added partnerName as dependency
